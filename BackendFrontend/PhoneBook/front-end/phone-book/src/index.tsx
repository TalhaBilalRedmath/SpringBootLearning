          ...
          {contacts.length === 0 ? (
            ...
            contacts.map(contact => (
              <tr key={contact.id} style={{ borderBottom: '1px solid #eee' }}>
                <td style={{ padding: 10 }}>{contact.id}</td>
                <td style={{ padding: 10 }}>{contact.name}</td>
                <td style={{ padding: 10 }}>{contact.number}</td>
                <td style={{ padding: 10 }}>
                  <button
                    onClick={() => updateContactPrompt(contact.id, contact.name, contact.number)}
                    style={{ marginRight: 8, background: '#f59e42', color: '#fff', border: 'none', borderRadius: 6, padding: '6px 12px', cursor: 'pointer', fontWeight: 500 }}
                  >
                    Update
                  </button>
                  <button
                    onClick={() => deleteContact(contact.name, contact.id)}
                    style={{ background: '#ef4444', color: '#fff', border: 'none', borderRadius: 6, padding: '6px 12px', cursor: 'pointer', fontWeight: 500 }}
                  >
                    Delete
                  </button>
                </td>
